  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: agents } = await supabase
        ...
        .select(`
          ...
          crm_clients (
            id,
            crm_policies (
              id, status, annual_premium, date_written
            )
          )
        `)
      ...
      const stats = (agents ?? []).map(agent => {
        const clients = agent.crm_clients ?? []
    ...
    <div>
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '12px', border: '1px solid #E5E1DA', overflow: 'hidden' }}>
        <div style={{ padding: '18px 24px', borderBottom: '1px solid #E5E1DA', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#1A1A1A' }}>Agent Leaderboard — MTD Production</h2>
        ...
        {agentStats.length > 0 ? (
              ...
              {agentStats.map((agent, i) => (
                <tr key={agent.id} style={{ borderBottom: i < agentStats.length - 1 ? '1px solid #F0EDE8' : 'none' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '26px', height: '26px', borderRadius: '50%', fontSize: '12px', fontWeight: '700', backgroundColor: i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : '#F0EDE8', color: i < 3 ? '#1A1A1A' : '#7A7A7A' }}>
                      {i + 1}
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px' }}>
          ...
          <div style={{ padding: '60px', textAlign: 'center' }}>
            <p style={{ fontSize: '15px', color: '#7A7A7A' }}>No active agents in the CRM yet.</p>
            <p style={{ fontSize: '13px', color: '#AAA', marginTop: '4px' }}>Agents appear here automatically when they reach Active status.</p>
          </div>
        )}
      </div>
    </div>
